...
def write_mask_to_disk(detected_mask, profile, output_fp, config):
    ...
    if config.validity_mask_fp is not None:
        ...
        with rasterio.open(f"/mnt/DiskA/xxzhang/Indnoesia/temp.tif", 'w', **profile) as out_ds:
            out_ds.write(detected_mask.astype(profile["dtype"]), 1)

        # Mask by valid areas
        with rasterio.open(f"/mnt/DiskA/xxzhang/Indnoesia/temp.tif") as src:
...
def predict_all(conf):
    ...
    for image_fp in tqdm(input_images, desc=f"{'Analysing images':<25}", position=0):
        ...
        with multiprocessing.Pool(processes=config.prediction_workers) as pool:
            with tqdm(total=len(params), desc="Processing image chunks", position=1, leave=False) as pbar:
                ...
                try:
                    ...
                    print(f"Error processing {image_fp}: {e}")
                pbar.update()
        ...
        if not chunk_fps:
            print(f"No image chunks created for {image_fp}. Skipping VRT build.")
            continue

        # Print out the chunk file paths for debugging
        print(f"Chunk file paths: {chunk_fps}")

        # Build VRT for merging
        vRunfile = os.path.join(config.prediction_output_dir, "merged.vrt")
        gdal.BuildVRT(vRunfile, chunk_fps)
        options = ["TILED=YES", "BIGTIFF=IF_SAFER", "COMPRESS=LZW", "NBITS=1" if config.output_dtype == "bool" else ""]

        if config.output_dtype == "bool":
            options.append("NBITS=1")
            
        gdal.Translate(output_file, vRunfile, creationOptions=options)
